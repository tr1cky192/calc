...
export default function Step3() {
 ...
 const handlePanelWidthsChange = (e) => {
  ...
  if (widths.length > 0 && !widths.includes(B)) {
    setB(widths[0]); // Якщо поточне B не в новому списку, вибрати перше значення
  }
};
...
if (profileOption === 'L1') {
  ...
  const roundedValue = Math.ceil(result / constants.L2 / 0.5) * 0.5; // Округлюємо до 0.5
  const integerPart = Math.floor(roundedValue); // Відкидаємо десяткову частину
...
localStorage.setItem(`roofingResult_${profileOption}`, JSON.stringify(savedResults))
  ...
  const handlePanelSelect = (panel) => {
    if (panel.availability === 'Немає в наявності') {
      // Don't allow selection of unavailable panels
  ...
  useEffect(() => {
    ...
    if (storedUser) {
      ...
      setUser(parsedUser); // якщо є useState для користувача
      setIsAdmin(parsedUser.role === 'administrator'); // перевірка ролі
    }
  }, []);
  const logout = () => {
    ...
    <div className="space-y-3 text-lg">
      ...
      {profileOption === 'L1_L2' ? (
        <>
          ...
          <p>
            <span className="font-medium">Загальна кількість профілів:</span> {NL1 + NL2} {/* Приклад обчислення загальної кількості для обох профілів */}
          </p>
        </>
      ) : (
        <p>
          <span className="font-medium">Загальна кількість профілів:</span> {L1N} {/* Лише для L1 */}
        </p>
      )}
      <p>
